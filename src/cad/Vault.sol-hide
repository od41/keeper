...
interface IVault {
    ...
    function setMaxTokenLeverage(address _token, uint256 _maxLeverage) external;
    ...
    function maxTokenLeverage(address _token) external view returns (uint256);
...
// File contracts/csr/interfaces/ITurnstile.sol
...
contract Vault is ReentrancyGuard, IVault {
    ...
    // max leverage amounts per token, if not set will revert to global max leverage
    mapping(address => uint256) public override maxTokenLeverage;
    ...
    function setMaxTokenLeverage(
        address _token,
        uint256 _maxLeverage
    ) external override {
        _onlyGov();
        maxTokenLeverage[_token] = _maxLeverage;
    }
